...
fn create_rng_scalar_polynomial<R, D>(
    num_samples: usize,
    rng: &mut R,
    dist: &D,
) -> Polynomial<f64>
...
fn benchmark(c: &mut Criterion) {
    ...
    let scalar_poly_arr= [
        create_rng_scalar_polynomial(3, &mut rng, &f64_dist),
        create_rng_scalar_polynomial(4, &mut rng, &f64_dist),
        create_rng_scalar_polynomial(5, &mut rng, &f64_dist),
    ];

    let complex_poly_arr = [
        create_rng_complex_polynomial(3, &mut rng, &f64_dist),
        create_rng_complex_polynomial(4, &mut rng, &f64_dist),
